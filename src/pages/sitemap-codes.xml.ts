...
export const GET: APIRoute = async ({ site }) => {
  ...
  for (const t of Array.from(typeSet).sort()) urls.push({ loc: `${base}/appliances/${t}/`, lastmod: isoDate() });
  ...
  for (const e of systems) {
    const codeSlug = e.slug.split('/').slice(-1)[0];
    urls.push({ loc: `${base}/systems/${e.data.subcategory}/error-codes/${codeSlug}/`, lastmod: isoDate(e.data.lastmod) });
  }
